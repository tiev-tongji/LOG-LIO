...
struct SimpInfo {
    float s_xx = 0.0, s_yy = 0.0, s_zz = 0.0, s_xy = 0.0, s_xz = 0.0, s_yz = 0.0;
    Eigen::Vector3f normal, mean_xyz;
    int num_points = 0;
};
...
//        pthread_mutex_init(&updating_mutex_lock, NULL);
    ...
    VoxelInfo(const PointType & p);

    void init();

    void updateWithSingle(const PointType & p);
    void updateWithSingle(const PointType & p, const Eigen::Matrix3f & cov);

    void updateWithCloud();
    void updateSumAndMean(const PointType & p);
    void updatePlaneUncertainty(const PointType & p, const Eigen::Matrix3f & cov);
    void updateNormal(const PointType & p);
    bool checkPCA();
    void updatePlaneCov();

    void getSimpInfo(SimpInfo& out);
...
public:
    ...
    struct PointType_Voxel_CMP{
        PointType point;
        float dist = 0.0;
        VoxelInfoPtr info = nullptr;

        PointType_Voxel_CMP (PointType p = PointType(), VoxelInfoPtr n = nullptr, const float& d = INFINITY){
            this->point = p;
            this->dist = d;
            this->info = n;
        };
        bool operator < (const PointType_Voxel_CMP &a)const{
            if (fabs(dist - a.dist) < 1e-10) return point.x < a.point.x;
            else return dist < a.dist;
        }
    };

    class MANUAL_VOXEL_HEAP{
    public:
        MANUAL_VOXEL_HEAP(int max_capacity = 100){
            cap = max_capacity;
            heap = new PointType_Voxel_CMP[max_capacity];
            heap_size = 0;
        }

        ~MANUAL_VOXEL_HEAP(){ delete[] heap;}
        ...
        PointType_Voxel_CMP top(){ return heap[0];}

        void push(PointType_Voxel_CMP point){
    ...
    private:
        int heap_size = 0;
        int cap = 0;
        PointType_Voxel_CMP * heap;
        void MoveDown(int heap_index){
            int l = heap_index * 2 + 1;
            PointType_Voxel_CMP tmp = heap[heap_index];
            while (l < heap_size){
    ...
    struct PointType_CMP{
        PointType point;
        float dist = 0.0;
        KD_TREE_NODE* node = nullptr;
//        PointType_CMP (PointType p = PointType(), float d = INFINITY){
//            this->point = p;
//            this->dist = d;
//        };
        PointType_CMP (PointType p = PointType(),  KD_TREE_NODE* n = nullptr, const float& d = INFINITY){
            this->point = p;
            this->dist = d;
            this->node = n;
        };
...
private:
    ...
    float calc_dist(const PointType& a, const Eigen::Vector3f& b);
    float calc_dist(const PointType& a, const float& x, const float& y, const float& z);
    float calc_dist(PointType a);
    ...
    void buildConnection(KD_TREE_NODE * root, const bool & traverse = true); //build connection between ikdtree nodes and voxels
    void single_connect(KD_TREE_NODE * root);
    void point2Voxel(const PointType& p, VOXEL_LOC& voxel);
    void point2Voxel(const PointType& p, float* loc_xyz);
    void point2Voxel(const PointType& p, float* loc_xyz, float* center_xyz);
    void point2Voxel(const PointType& p, VOXEL_LOC& voxel, float* center_cube);
    void voxelIndexNode(const PointType& p, VOXEL_LOC& voxel);
    void node2Voxel(KD_TREE_NODE * root, VOXEL_LOC& voxel);
    void insideVoxelDownsample(PointVector& point_cloud);
    void voxelLoc2Center(const float* loc_xyz, float* center_xyz);
    void addNearVoxelsLoc(const PointType& point, const float* voxel_center, std::vector<float>& voxels);
public:
    ...
    void Nearest_Search(PointType point, int k_nearest, PointVector &Nearest_Points, vector<float> & Point_Distance, vector<VoxelInfoPtr>& voxels, double max_dist = INFINITY);
    ...
    void checkCorrespondding(KD_TREE_NODE * root);
//    int checkCorresponddingVoxelAndNode(KD_TREE_NODE * root);
    void acquire_removed_points(PointVector & removed_points);
//    void updateNodeInfo(KD_TREE_NODE* node, const PointType& point);

    void Voxel_Search(const PointType& point, int k_nearest, PointVector &Nearest_Points, vector<KD_TREE_NODE*>& nodes);
    void Voxel_Search_test(const PointType& point, int k_nearest, PointVector &Nearest_Points, vector<float> & Point_Distance, vector<VoxelInfoPtr>& voxels);

    void updateVoxelInfo(const PointType& point);
    void updateVoxelInfo(const PointType& point, const Eigen::Matrix3f & cov);

    //voxel map
//    unordered_map<VOXEL_LOC, OctoTree *> voxel_map;
    unordered_map<VOXEL_LOC, KD_TREE_NODE *> voxel2node;
    unordered_map<VOXEL_LOC, VoxelInfo*> voxel2info;
//    unordered_map<KD_TREE_NODE *, VOXEL_LOC> node2voxel;
    void findVoxel(const PointType& p);
    int max_layer, layer, max_cov_points_size, max_points_size;
    vector<int> layer_point_size;
    float planer_threshold;
    float voxel_size;
    double max_voxel_size, min_eigen_value, half_voxel_size, quater_voxel_size;
    int voxel_conflict = 0;
    int max_voxel_points_size;
    ...
    //check corresponding
    int num_node2_voxel_null = 0, num_voxel2node_null = 0;
    int num_nodes = 0;
    int voxel2info_null = 0;
    int num_same_info = 0, num_diff_info = 0;
    int num_same_voxel2node = 0;
    int num_node2voxel_valid = 0, voxel2info_valid = 0;
    int num_same_voxel_by_pointAndnode = 0;
};
